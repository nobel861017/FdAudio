...
class FluxAudio(nn.Module):
    ...
    def load_weights(self, src_dict) -> None:
        ...
        # If LoRA has been injected, the model carries extra lora_A/lora_B keys
        # absent from a vanilla pretrained checkpoint — load non-strictly so the
        # base weights still populate and the adapters keep their init.
        has_lora = any('lora_' in k for k in self.state_dict().keys())
        self.load_state_dict(src_dict, strict=not has_lora)

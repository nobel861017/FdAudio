...
class MeanFlow():
    ...
    def predict_x1_one_step(self, fn: Callable, x0: torch.Tensor) -> torch.Tensor:
        # Gradient-enabled NFE=1 generation. fn(t, r, x) takes scalar-broadcast t,r tensors.
        t = torch.ones((x0.shape[0],), device=x0.device, dtype=x0.dtype)
        r = torch.zeros((x0.shape[0],), device=x0.device, dtype=x0.dtype)
        u_flow = fn(t=t, r=r, x=x0)
        return x0 - u_flow

    def predict_x1_n_steps(self, fn: Callable, x0: torch.Tensor, n: int) -> torch.Tensor:
        # Gradient-enabled NFE=n generation (mirror of run_t0_to_t1 without no_grad).
        # Used by FD post-training to optimize multi-step trajectories.
        if n == 1:
            return self.predict_x1_one_step(fn, x0)
        steps = torch.linspace(1, 0, n + 1, device=x0.device, dtype=x0.dtype)
        x = x0
        for i in range(n):
            t = steps[i].expand(x0.shape[0])
            r = steps[i + 1].expand(x0.shape[0])
            u_flow = fn(t=t, r=r, x=x)
            dt = (t - r).mean()
            x = x - dt * u_flow
        return x

    def predict_x1_sub_traj(self, fn: Callable, x0: torch.Tensor, n: int) -> torch.Tensor:
        """Sub-trajectory NFE=n generation with single-step gradient.

        Picks a random sub-step k ∈ [0, n); steps before k run in no_grad,
        step k runs with gradient, steps after k run with detached model output
        (preserves x's grad lineage but no model-param gradient through them).
        Memory: same as NFE=1 backward. Compute: ~n model forwards + 1 backward.
        Bias: phase-3 Jacobian is treated as identity (model in continuation
        frozen for the purpose of gradient flow), but the loss still depends on
        the actually-evolved final x_1. In expectation over random k, every
        (r, t) interval gets gradient updates.
        """
        import random as _random
        if n == 1:
            return self.predict_x1_one_step(fn, x0)
        steps = torch.linspace(1, 0, n + 1, device=x0.device, dtype=x0.dtype)
        k = _random.randint(0, n - 1)

        x = x0
        # Phase 1: no_grad warm-up
        with torch.no_grad():
            for i in range(k):
                t = steps[i].expand(x0.shape[0])
                r = steps[i + 1].expand(x0.shape[0])
        ...
        # Phase 2: the one step that carries gradient
        t = steps[k].expand(x0.shape[0])
        r = steps[k + 1].expand(x0.shape[0])
        u = fn(t=t, r=r, x=x)
        dt = (t - r).mean()
        x = x - dt * u

        # Phase 3: continuation — model in no_grad on detached input so u has no
        # grad, but the outer subtraction preserves x's grad lineage from phase 2.
        for i in range(k + 1, n):
            t = steps[i].expand(x0.shape[0])
            r = steps[i + 1].expand(x0.shape[0])
            with torch.no_grad():
                u = fn(t=t, r=r, x=x.detach())
            dt = (t - r).mean()
            x = x - dt * u
        return x

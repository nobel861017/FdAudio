...
class FeaturesUtils(nn.Module):
    ...
    def decode_with_grad(self, z: torch.Tensor) -> torch.Tensor:
        # Gradient-enabled latent → mel. Bypass tod.decode's @torch.inference_mode wrapper.
        # Underlying VAE params remain frozen via requires_grad=False set in AutoEncoderModule.
        assert self.tod is not None, 'VAE is not loaded'
        return self.tod.vae.decode(z.transpose(1, 2))

    def vocode_with_grad(self, mel: torch.Tensor) -> torch.Tensor:
        # Gradient-enabled mel → waveform. Both tod.vocode AND BigVGAN.forward are
        # @torch.inference_mode wrapped, so we call BigVGAN.vocoder (the inner BigVGANVocoder)
        # directly to bypass both layers. Gradient checkpointing saves vocoder activation
        # memory at the cost of one extra forward during backward — required to fit
        # multi-NFE FD post-training on Blackwell.
        assert self.tod is not None, 'VAE is not loaded'
        if not mel.requires_grad:
            return self.tod.vocoder.vocoder(mel)
        from torch.utils.checkpoint import checkpoint
        return checkpoint(self.tod.vocoder.vocoder, mel, use_reentrant=False)

...
def _load_audio(path: str, *, start: int = 0, frames: int = -1):
    """Load audio via soundfile with optional partial read.

    Partial reading is critical for WavCaps — some FreeSound FLACs are >1GB
    and exceed worker RAM when fully decoded.
    """
    data, sr = sf.read(str(path), start=start, frames=frames,
...
class WavTextClipsDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        try:
            ...
            audio_chunk, sample_rate = _load_audio(
                audio_path, start=start_sample, frames=needed)
            ...
            if self.reject_silent and abs_max < 1e-6:
                ...
                return None
            if audio_chunk.size(0) < needed:
                audio_chunk = F.pad(
                    audio_chunk,
                    (0, needed - audio_chunk.size(0)),
                    mode='constant',
                    value=0
                )

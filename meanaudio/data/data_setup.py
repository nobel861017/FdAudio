...
def setup_training_datasets(cfg: DictConfig) -> tuple[Dataset, DistributedSampler, DataLoader]:
    ...
    if cfg.mini_train: 
        ...
        if cfg.get('use_wavcaps', False):
            audioset_sl_npz = load_audio_data(cfg, cfg.data.AudioSetSL_npz)
            bbcsound_npz   = load_audio_data(cfg, cfg.data.BBCSound_npz)
            freesound_npz  = load_audio_data(cfg, cfg.data.FreeSound_npz)
            soundbible_npz = load_audio_data(cfg, cfg.data.SoundBible_npz)
            dataset = MultiModalDataset(
                [],
                [audiocaps_npz] * cfg.ac_oversample_rate
                + [audioset_sl_npz, bbcsound_npz, freesound_npz, soundbible_npz],
            )
        else:

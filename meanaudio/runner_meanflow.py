...
class RunnerMeanFlow:
    ...
    def __init__(self,
        ...
        _net = get_mean_audio(cfg.model,  # get the model based on base_config.yaml
        ...
        self.lora_enabled = bool(lora_cfg is not None and lora_cfg.get('enable', False))
        if self.lora_enabled:
            from meanaudio.model.lora import inject_lora, mark_only_lora_trainable
            rank = int(lora_cfg.get('rank', 16))
            alpha = float(lora_cfg.get('alpha', rank))
            targets = tuple(lora_cfg.get('targets', ['qkv']))
            n_lora = inject_lora(_net, rank=rank, alpha=alpha, targets=targets)
            mark_only_lora_trainable(_net)
            log.info(f'LoRA enabled: rank={rank} alpha={alpha} targets={targets} '
                     f'-> {n_lora} layers wrapped')
        self.network = DDP(_net,
        ...
        if for_training:
            ...
            self.save_weights_copy_interval = cfg.get('save_weights_copy_interval', 0)
    ...
    def train_pass(self, data, it: int = 0):
        ...
        if self.save_weights_copy_interval > 0 and it % self.save_weights_copy_interval == 0 and it != 0:
            save_copy = True
    ...
    def save_weights(self, it, save_copy=False):  # only save net's weights
        ...
        # With LoRA, save a merged plain checkpoint so the eval pipeline (which
        # builds a vanilla, non-LoRA model) can load it unchanged.
        if getattr(self, 'lora_enabled', False):
            from meanaudio.model.lora import lora_merged_state_dict
            state = lora_merged_state_dict(self.network.module)
        else:
            state = self.network.module.state_dict()
        ...
        if save_copy:
            ...
            # Also save the live Post-Hoc EMA weights (default sigma_rel =
            # sigma_rels[0]) as a directly-loadable plain checkpoint, so EMA can
            # be evaluated/selected per-iter. Kept in a subdir to stay out of the
            # raw-weight watcher's top-level glob. Full-FT only (LoRA EMA would
            # need merging; our EMA-study configs are full fine-tune).
            if (self.ema is not None and it >= self.ema_start
                    and not getattr(self, 'lora_enabled', False)):
                ema_dir = self.run_path / 'ema_weights'
                os.makedirs(ema_dir, exist_ok=True)
                ema_state = self.ema.ema_models[0].ema_model.state_dict()
                ema_path = ema_dir / f'{self.exp_id}_ema_{it}.pth'
                torch.save(ema_state, ema_path)
                self.log.info(f'EMA weights (sigma_rel={self.cfg.ema.sigma_rels[0]}) '
                              f'saved to {ema_path}.')
